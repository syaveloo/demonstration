...
class Program:
    ...
    def create_keyboards(self):
        ...
        self.keyboard_default_chat.add_button("@achedz", color=VkKeyboardColor.NEGATIVE)
        self.keyboard_default_chat.add_button("@achedz обнови", color=VkKeyboardColor.SECONDARY)
        
        # prikol ketboard
        self.keyboard_prikol = VkKeyboard(one_time=False)
        self.keyboard_prikol.add_button("хихих", color=VkKeyboardColor.POSITIVE)
    ...
    def listener(self):   
        for event in self.longpoll.listen():
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                # Беседа
                if event.from_chat and '@all' not in text:
                    id = event.chat_id
                    print(text, "in chat:", id)
                    print(time.time()-Program.last_message_time)
                    if time.time()-Program.last_message_time > 60.0:
                        Program.last_message_time = time.time()
                        # дз
                        if "обнови" in text:
                            if time.time()-Program.last_update_time > 1800.0:
                                ...
                                Program.send(self, id=id, text="хихихи", keyboard=None, receiver='chat')
                        else:
                            Program.send(self, id=id, text=Homework().Get(text,noperms=1), keyboard=self.keyboard_default_chat, receiver='chat')
                    else:
                        Program.send(self, id=id, text='хихих', keyboard=self.keyboard_prikol, receiver='chat')
                # ЛС
                    ...
                    if time.time()-Program.last_message_time > 60.0:
                        ...
                        if "ачедз" in text:
                            ...
                            if time.time()-Program.last_update_time > 1800.0:
                                ...
                                Program.send(self, id=id, text="хихих", keyboard=None, receiver='user')
                        else:
                            Program.send(self, id=id, text=Homework().Get(text,noperms=1), keyboard=None, receiver='user')
                    else:
                        Program.send(self, id=id, text='хихих', keyboard=self.keyboard_prikol, receiver='user')
